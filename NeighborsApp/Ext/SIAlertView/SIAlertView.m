...
//        UIImage *blurredSnapshot = [[UIImage imageWithData:imageData] blurredImage:0.9];
        
//        UIImageView *BgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
//        BgView.backgroundColor = [UIColor blackColor];
//        BgView.image = blurredSnapshot;
//        BgView.alpha = 0.7f;
        
        UIView *blackBgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
        blackBgView.backgroundColor = [UIColor colorWithRed:0.7 green:0.7 blue:0.7 alpha:0.7f];
...
+ (void)initialize
{
    ...
    appearance.titleColor = [UIColor colorWithRed:241.0/255 green:90.0/255 blue:34.0/255 alpha:1.0];
    appearance.titleFont = [UIFont fontWithName:kNBR_DEFAULT_FONT_NAME_BLOD size:16.0f];

    appearance.messageColor = kNBR_ProjectColor_DeepGray;
    appearance.messageFont = [UIFont fontWithName:kNBR_DEFAULT_FONT_NAME_BLOD size:14.0f];
    
    appearance.buttonFont = [UIFont fontWithName:kNBR_DEFAULT_FONT_NAME size:14];
...
- (void)validateLayout
{
	...
	if (self.titleLabel) {
        ...
        CGFloat width = [self widthForTitleLable];
        
        self.titleLabel.frame = CGRectMake(CONTENT_PADDING_LEFT * 2 + (self.containerView.frame.size.width / 2.0f - (width + 22) / 2.0f), y + 2, width, height);
        y += height;
        
        self.iconView.frame = CGRectMake(self.titleLabel.frame.origin.x - 22, 12, 20, 20);
	}
...
- (void)updateTitleLabel
{
	if (self.title) {
        ...
        self.iconView.image = [UIImage imageNamed:@"excalmatoryIcon"];
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
    ...
    button.titleLabel.font = [UIFont fontWithName:kNBR_DEFAULT_FONT_NAME_BLOD size:14.0f];
