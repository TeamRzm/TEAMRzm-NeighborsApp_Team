...
#define SAMPLES_PER_SECOND	8000.00f

// Derive the Buffer Size. I punt with the max buffer size.
void DeriveBufferSize (AudioQueueRef audioQueue, AudioStreamBasicDescription ASBDescription, Float64 seconds, UInt32 *outBufferSize)
{
    static const int maxBufferSize = 0x5000; // punting with 5k
...
{
    [self readyForStartRecodring:filePath];
...
- (void) reallyStopRecording
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(recoderDidStopRecodeingWithAudioLength:)])
    {
        NSTimeInterval sec = [recordStartTime timeIntervalSinceNow];
        
        [self.delegate recoderDidStopRecodeingWithAudioLength:-sec];
    }
